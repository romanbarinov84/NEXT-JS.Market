...
const fetchProductsCardCategory = async (category:string,
  options: {
  ...
  try{
    ...
    if(!res.ok) {
        throw new Error (`Помилка сервера при доступі до категорій`);
    }
    ...
    console.error("Помилка загрузки категорій", error);
  }

    
}

export default fetchProductsCardCategory;